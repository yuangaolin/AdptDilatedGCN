...
class ModelNew(nn.Module):
    ...
    def forward(self, ga, gr, gi, vina):
        ...
        # pocket-ligand node features
        vi_init = torch.cat((va, vr), dim=0)
...
class Adaptive_GCN(nn.Module):
    def __init__(self, v_dim, e_dim, h_dim, k_head):
        ...
        self.gate_update_m = Adaptive_GCN.Adaptive_GRU(h_dim)
        self.gate_update_s = Adaptive_GCN.Adaptive_GRU(h_dim)
    ...
    class Adaptive_GRU(nn.Module):

        def __init__(self, h_dim):
            super().__init__()
            self.gate_weights = nn.Parameter(torch.Tensor(h_dim))
            self.A = nn.Linear(h_dim, h_dim)
            self.B = nn.Linear(h_dim, h_dim)
            self.gru = nn.GRUCell(h_dim, h_dim)

        def forward(self, a, b, c):
            z = torch.sigmoid(self.A(a) + self.B(b) + self.gate_weights)
            h = z * b + (1 - z) * a
            cc = self.gru(c, h)
            return cc
...
class Dilated_GCN(nn.Module):
    def __init__(self, v_dim, e_dim, h_dim, k_head):
        ...
        self.D = nn.Linear(e_dim + v_dim, h_dim)
        self.E = nn.Linear(h_dim + v_dim, h_dim)
        self.gate_update_m = Dilated_GCN.Adaptive_GRU(h_dim)
        self.gate_update_s = Dilated_GCN.Adaptive_GRU(h_dim)
